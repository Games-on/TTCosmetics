(function ($) {
    ...
    // Logic xử lý phần ô tăng giảm sản phẩm và số tiền = jquery
    $('.quantity button').on('click', function () {
        let change = 0;

        var button = $(this); // lấy ra phần tử vừa click vào
        var oldValue = button.parent().parent().find('input').val(); // lấy ra giá trị của ô input
        if (button.hasClass('btn-plus')) {
            var newVal = parseFloat(oldValue) + 1;
            change = 1;
        } else {
        ...
        const priceElement = $(`p[data-cart-detail-id='${id}']`); // đi tìm element muốn update
        if (priceElement) {
        ...
        const totalPriceElement = $(`p[data-cart-total-price]`); // lấy ra phần tử data cart price
